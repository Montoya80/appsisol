...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  Future<void> _fetchUsers() async {
    ...
    try {
      ...
      // Fetch schedules
      final schedulesData = await Supabase.instance.client.from('schedules').select().order('name');
      if (mounted) {
        setState(() => _schedules = List<Map<String, dynamic>>.from(schedulesData));
      }
    } catch (e) {
      if (mounted) {
  ...
  void _showUserForm({Map<String, dynamic>? user}) {
    ...
    String? selectedScheduleId = user?['schedule_id'];
    
    // Horarios Laborales
    final workStartController = TextEditingController(text: user?['work_start_time'] ?? '09:00:00');
    final workEndController = TextEditingController(text: user?['work_end_time'] ?? '18:00:00');
          ...
          builder: (context, setDialogState) {
            ...
            final Widget generalSection = Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(labelText: 'Horario Asignado', prefixIcon: Icon(Icons.schedule)),
                  items: [
                    const DropdownMenuItem<String>(value: null, child: Text('SIN HORARIO ASIGNADO')),
                    ..._schedules.map((s) => DropdownMenuItem<String>(
                      value: s['id'], 
                      child: Text('${s['name']} (${s['zone'] ?? ""})'.trim(), overflow: TextOverflow.ellipsis)
                    )),
                  ],
                  onChanged: (v) => setDialogState(() => selectedScheduleId = v),
                ),
                const SizedBox(height: 16),
                TextField(
            ...
            final Widget permissionsSection = Column(
              ...
              children: [
                ...
                const Divider(),
                _buildPermissionSwitch('Checador (App)', 'show_checador', Icons.fact_check, permissions, setDialogState),
                _buildPermissionSwitch('Asistencias (Admin)', 'show_asistencias', Icons.rule, permissions, setDialogState),
                _buildPermissionSwitch('Horarios (Config)', 'show_horarios', Icons.event_note, permissions, setDialogState),
              ],
            );
                          ...
                          child: ElevatedButton(
                            ...
                            try {
                              if (isEditing && !isGrantingAccess) {
                                ...
                                // Actualizar Horarios en Perfil
                                await Supabase.instance.client.from('profiles').update({
                                  'work_start_time': workStartController.text.trim(),
                                  'work_end_time': workEndController.text.trim(),
                                  ...
                                  try {
                                    await NotificationService.send(
                                      ...
                                      metadata: {
                                        'profile_id': user['id'],
                                        'new_status_sys': statusSys,
                                        'new_status_rh': user?['status_rh'] ?? 'ACTIVO',
                  ...
                  Row(
                    children: [
                      ...
                      if (user['permissions'] != null) ...[
                        ...
                        _buildMiniIcon(Icons.fact_check, user['permissions']['show_checador'] == true),
                        _buildMiniIcon(Icons.rule, user['permissions']['show_asistencias'] == true),
                        _buildMiniIcon(Icons.event_note, user['permissions']['show_horarios'] == true),
                      ],
                    ],
                  ),
                ],
