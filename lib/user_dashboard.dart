...
class _UserDashboardState extends State<UserDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    final scheduleData = _profile?['schedules'];
    final scheduleCard = Card(
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.schedule_outlined, color: theme.colorScheme.primary, size: 28),
            ...
            if (scheduleData != null) ...[
              ...
              const SizedBox(height: 12),
              ...(() {
                final Map<int, Map<String, dynamic>> groupedRules = {};
                for (var r in (scheduleData['rules'] as List? ?? [])) {
                  final day = r['day'] as int;
                  groupedRules.putIfAbsent(day, () => {'day': day});
                  if (r['type'] == 'ENTRADA') {
                    groupedRules[day]!['in'] = r['time'];
                    groupedRules[day]!['tol'] = r['tol'];
                  } else {
                    groupedRules[day]!['out'] = r['time'];
                  }
                }
                final sortedDays = groupedRules.keys.toList()..sort((a, b) => (a == 0 ? 7 : a).compareTo(b == 0 ? 7 : b));

                return sortedDays.map((day) {
                  final r = groupedRules[day]!;
                  final dayName = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'][day];
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            '$dayName: ${r['in']?.toString().substring(0, 5) ?? "--:--"} - ${r['out']?.toString().substring(0, 5) ?? "--:--"}${r['tol'] != null ? " (T: ${r['tol']}m)" : ""}',
              ...
              if (scheduleData['zone'] != null && scheduleData['zone'].toString().isNotEmpty) ...[
                const Divider(),
                Text(
                  'Zona: ${scheduleData['zone']}',
                  style: const TextStyle(fontSize: 12, fontStyle: FontStyle.italic, color: Colors.grey),
                ),
              ],
            ] else ...[
              const Text(
                'Sin horario maestro asignado.',
